<?php
// Simple PostgreSQL Diagnostic - No framework dependencies
...
<body>
    ...
    <h2>Next Steps</h2>
    <pre>1. If connection is successful:
   → Go to DATA2REST create database form
   → Fill in the exact same credentials
   → Test connection should work

2. If 'pdo_pgsql driver is NOT available':
   → You need to install PHP PostgreSQL extension
   → For ServBay: Check ServBay PHP extensions settings
   → For Homebrew PHP: brew install php-pgsql

3. If 'database does not exist':
   → Run: cd /opt/homebrew/var/www/data2rest
   → Run: ./scripts/create_pg_database.sh mi_tienda
</pre>
</body>
