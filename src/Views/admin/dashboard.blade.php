...
@section('content')
    ...
    @if(\App\Core\Auth::isAdmin() && !class_exists('Google\Client'))
        <div
            class="animate-in fade-in slide-in-from-top-4 duration-500 mb-8 p-6 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex flex-col md:flex-row items-center gap-6 relative overflow-hidden group">
            <div
                class="absolute inset-0 bg-amber-500/5 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000">
            </div>
            <div class="flex-shrink-0 w-12 h-12 rounded-xl bg-amber-500/20 flex items-center justify-center text-amber-500">
            ...
            <div class="flex-1 text-center md:text-left z-10">
                <h3 class="text-sm font-black text-amber-500 uppercase tracking-widest mb-1">New Feature Available: Google Login
                </h3>
                <p class="text-xs text-p-muted font-medium">
                    To enable the new Google Login feature, you need to install the required PHP library.
                </p>
            </div>
            <div class="flex flex-col md:flex-row items-center gap-4 z-10 w-full md:w-auto">
                <code
                    class="px-3 py-1.5 rounded-lg bg-black/40 border border-white/10 text-[10px] font-mono text-amber-200 whitespace-nowrap">
                            composer require google/apiclient
                        </code>
                <button onclick="navigator.clipboard.writeText('composer require google/apiclient')"
                    class="p-2 text-amber-500 hover:bg-amber-500/20 rounded-lg transition-colors" title="Copy Command">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
        ...
        <div class="glass-card py-8 flex flex-col items-center border-b-4 border-amber-500/50 group relative">
            ...
            @endif
            <span class="text-4xl font-black text-p-title mb-1">{{ $stats['storage_usage'] }}</span>
            <span class="text-[10px] font-black text-p-muted uppercase tracking-[0.2em] mb-4">
                {{ \App\Core\Lang::get('dashboard.stats.storage') }}
            </span>
            @if($storage)
                <div class="text-[9px] font-black uppercase tracking-widest text-amber-500/80">
                    {{ $storage['used_mb'] }} MB / {{ $storage['quota_mb'] }} MB Quota
                </div>
            @endif
        </div>
    </div>
        ...
        <div class="lg:col-span-2 glass-card !p-8 border-t-4 border-primary/30 relative overflow-hidden group">
            ...
            <div class="flex justify-between items-center mb-8 relative z-10">
                ...
                <div class="flex gap-2">
                    <span
                        class="flex items-center gap-2 text-[9px] font-black uppercase text-primary bg-primary/10 px-3 py-1.5 rounded-full border border-primary/20">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16">
        ...
        <div class="glass-card !p-8 border-t-4 border-emerald-500/30">
            <div class="flex justify-between items-center mb-8">
                ...
                <span
                    class="text-[10px] font-black italic text-emerald-500 uppercase tracking-widest">+{{ array_sum($chartData['growth']['data']) }}
                    (7d)</span>
            </div>
        ...
        <div class="glass-card !p-8 border-t-4 border-blue-500/30 flex flex-col justify-center">
            <div class="absolute -right-20 -top-20 w-64 h-64 bg-blue-500/5 blur-[80px] rounded-full -z-10">
            </div>
            <div class="flex items-center gap-6">
                ...
                <div>
                    <h4 class="text-lg font-black text-p-title uppercase tracking-tighter italic">
                        {{ \App\Core\Lang::get('dashboard.ai_ready') }}
                    </h4>
            ...
            <div class="mt-8 pt-8 border-t border-glass-border flex gap-4">
                <div class="flex-1">
                    <span
                        class="block text-[10px] font-black text-p-muted uppercase tracking-widest mb-1">{{ \App\Core\Lang::get('dashboard.health_status') }}</span>
                    <span
                        class="text-sm font-black text-emerald-500 uppercase italic tracking-tighter">{{ \App\Core\Lang::get('dashboard.optimized') }}</span>
                </div>
                <div class="flex-1">
                    <span
                        class="block text-[10px] font-black text-p-muted uppercase tracking-widest mb-1">{{ \App\Core\Lang::get('dashboard.latency') }}</span>
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <a href="{{ $baseUrl }}admin/webhooks"
                    class="glass-card group hover:scale-[1.02] hover:border-primary/50 !p-8">
                    <div
                        class="w-12 h-12 bg-purple-500/10 rounded-xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500 text-purple-400">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                d="M13 10V3L4 14h7v7l9-11h-7z"></path>
                        </svg>
                    </div>
                    <h3 class="text-xl font-bold text-p-title mb-2">{{ \App\Core\Lang::get('webhooks.title') }}</h3>
                    <p class="text-xs text-p-muted mb-6 leading-relaxed">
                        {{ \App\Core\Lang::get('webhooks.subtitle') }}
                    </p>
                    <div class="text-[10px] font-black text-primary uppercase tracking-widest flex items-center gap-2">
                        {{ \App\Core\Lang::get('dashboard.enter') }} <span>&rarr;</span>
                    </div>
                ...
                <!-- Backups Module -->
                <a href="{{ $baseUrl }}admin/backups"
                    class="glass-card group hover:scale-[1.02] hover:border-primary/50 !p-8">
                    <div
                        class="w-12 h-12 bg-indigo-500/10 rounded-xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500 text-indigo-400">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path>
                        </svg>
                    </div>
                    <h3 class="text-xl font-bold text-p-title mb-2">{{ \App\Core\Lang::get('backups.title') }}</h3>
                    <p class="text-xs text-p-muted mb-6 leading-relaxed">
                        {{ \App\Core\Lang::get('backups.subtitle') }}
                    </p>
                    <div class="text-[10px] font-black text-primary uppercase tracking-widest flex items-center gap-2">
                        {{ \App\Core\Lang::get('dashboard.enter') }} <span>&rarr;</span>
                    </div>
                ...
                <!-- Billing Module -->
                @if(\App\Core\Auth::isAdmin() || $_SESSION['role_id'] == 4)
                    <a href="{{ $baseUrl }}admin/billing"
                        ...
                        <h3 class="text-xl font-bold text-p-title mb-2">Billing</h3>
                        <p class="text-xs text-p-muted mb-6 leading-relaxed">
                            {{ \App\Core\Auth::isAdmin() ? 'Gestión de pagos, cuotas y facturación por proyecto. Control financiero completo.' : 'Consulta tus cuotas, historial de pagos y reporta nuevos comprobantes.' }}
                        </p>
                ...
                <a href="{{ $baseUrl }}admin/tasks"
                    ...
                    <h3 class="text-xl font-bold text-p-title mb-2">Gestión de Tareas</h3>
                    <p class="text-xs text-p-muted mb-6 leading-relaxed">
                        Tablero Kanban para organizar y gestionar tareas del proyecto. Drag & Drop con control de permisos.
                    </p>
                    <div class="text-[10px] font-black text-purple-400 uppercase tracking-widest flex items-center gap-2">
                        {{ \App\Core\Lang::get('dashboard.enter') }} <span>&rarr;</span>
                    </div>
                </a>

                <!-- Recycle Bin -->
                <a href="{{ $baseUrl }}admin/trash"
                    class="glass-card group hover:scale-[1.02] hover:border-red-500/50 !p-8">
                    <div
                        class="w-12 h-12 bg-red-500/10 rounded-xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500 text-red-500">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                    ...
                    <h3 class="text-xl font-bold text-p-title mb-2">Recycle Bin</h3>
                    <p class="text-xs text-p-muted mb-6 leading-relaxed">
                        Recover records that were deleted by mistake. Supports multi-database rollback.
                    </p>
                    <div class="text-[10px] font-black text-red-500 uppercase tracking-widest flex items-center gap-2">
                        {{ \App\Core\Lang::get('dashboard.enter') }} <span>&rarr;</span>
                    </div>
                </a>

                <!-- System Database Administration (Super Admin Only) -->
                @if(\App\Core\Auth::isAdmin())
                    <a href="{{ $baseUrl }}admin/system-database"
                        class="glass-card group hover:scale-[1.02] hover:border-red-500/50 !p-8">
                        <div
                            class="w-12 h-12 bg-red-500/10 rounded-xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500 text-red-500">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <h3 class="text-xl font-bold text-p-title mb-2">{{ \App\Core\Lang::get('system_database.title') }}</h3>
                        <p class="text-xs text-p-muted mb-6 leading-relaxed">
                            Administración completa de la base de datos del sistema. Solo Super Admin.
                        </p>
                        <div class="text-[10px] font-black text-red-500 uppercase tracking-widest flex items-center gap-2">
                            {{ \App\Core\Lang::get('dashboard.enter') }} <span>&rarr;</span>
                        </div>
            ...
            <div class="glass-card !p-6 space-y-6">
                ...
                @else
                    @foreach($stats['recent_activity'] as $act)
                        @php
                            $isAudit = isset($act['action']);
                            $dotColor = 'bg-primary';
                            if ($isAudit && $act['action'] === 'DELETE')
                                $dotColor = 'bg-red-500';
                            if ($isAudit && $act['action'] === 'UPDATE')
                                $dotColor = 'bg-amber-500';

                            // If it's from audit trail, we have db_id. Otherwise we might need to find it (but for now just link if available)
                            $historyUrl = isset($act['db_id'])
                                ? $baseUrl . "admin/crud/history?db_id={$act['db_id']}&table={$act['table']}&id={$act['id']}"
                                : null;
                        @endphp
                        <div class="relative pl-6 border-l border-white/5 group">
                            <div
                                class="absolute left-[-5px] top-1 w-2.5 h-2.5 rounded-full {{ $dotColor }} border-4 border-dark group-hover:scale-125 transition-transform">
                            </div>
                            <p class="text-[10px] font-black text-p-muted uppercase tracking-widest mb-1">
                                {{ date('M d, H:i', strtotime($act['date'])) }}
                            </p>

                            @if($historyUrl)
                                <a href="{{ $historyUrl }}" class="block">
                                    <p class="text-xs font-bold text-p-title mb-1 hover:text-primary transition-colors">
                                        {{ $act['label'] }}
                                    </p>
                                </a>
                            @else
    ...
    <div class="mt-16 pt-16 border-t border-glass-border">
        ...
        <div class="max-w-2xl mx-auto flex flex-wrap justify-center gap-6">
            @if(\App\Core\Auth::isAdmin())
                ...
                <button onclick="window.location.href='{{ $baseUrl }}admin/backups'"
                    class="px-8 py-4 rounded-xl border border-indigo-500/30 text-indigo-500 text-[10px] font-black uppercase tracking-[0.2em] hover:bg-indigo-500 hover:text-white transition-all duration-300">
                    ☁️ {{ \App\Core\Lang::get('backups.title') }}
                </button>
