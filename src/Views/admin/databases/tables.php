...
<header class="mb-12 flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
    <div class="flex flex-col gap-4">
        <div>
            <h1 class="text-5xl font-black text-p-title italic tracking-tighter uppercase">
                <?php echo Lang::get('tables.title'); ?>
            </h1>
    ...
    <?php if (App\Core\Auth::hasPermission('module:databases.create_table')): ?>
        <div class="glass-card !p-4 !px-6 border-primary/20 bg-primary/5 w-full md:w-auto">
            <h2 class="text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-3">
                <?php echo Lang::get('tables.init_table'); ?>
            </h2>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    <?php foreach ($tables as $tableName => $count): ?>
        <div class="glass-card group hover:scale-[1.02] hover:shadow-2xl hover:shadow-primary/5">
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="flex items-center gap-4">
                    <div
                        ...
                        <?php echo $count; ?>     <?php echo Lang::get('dashboard.stats.records'); ?>
                    </div>
                    <div class="flex gap-1 group-hover:opacity-100 opacity-20 transition-opacity">
                        <?php if (App\Core\Auth::hasPermission('module:databases.edit_table')): ?>
                            <a href="<?php echo $baseUrl; ?>admin/databases/fields?db_id=<?php echo $database['id']; ?>&table=<?php echo $tableName; ?>"
                                class="p-2 text-p-muted hover:text-primary" title="<?php echo Lang::get('common.edit'); ?>">
                        ...
                        <?php endif; ?>
                        <?php if (App\Core\Auth::hasPermission('module:databases.drop_table')): ?>
                            <button onclick="confirmDeleteTable('<?php echo $tableName; ?>')"
                                class="p-2 text-p-muted hover:text-red-500" title="<?php echo Lang::get('common.delete'); ?>">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
            ...
            <div class="flex gap-4">
                <a href="<?php echo $baseUrl; ?>admin/crud/list?db_id=<?php echo $database['id']; ?>&table=<?php echo $tableName; ?>"
