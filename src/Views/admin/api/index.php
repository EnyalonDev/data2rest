...
<header class="mb-12">
    <h1 class="text-5xl font-black text-white italic tracking-tighter mb-2">
        <?php echo Lang::get('api_control.title'); ?></h1>
    <p class="text-slate-500 font-medium"><?php echo Lang::get('api_control.subtitle'); ?></p>
</header>
...
<div class="grid lg:grid-cols-2 gap-8">
    ...
    <section class="glass-card flex flex-col h-full">
        <div class="flex justify-between items-center mb-8">
            <h2 class="text-xl font-black text-white uppercase tracking-tight">
                <?php echo Lang::get('api_control.tokens'); ?></h2>
            <span
                ...
                <?php echo Lang::get('api_control.active'); ?></span>
        </div>

        <form action="<?php echo $baseUrl; ?>admin/api/keys/create" method="POST" class="mb-8 flex gap-2">
            <input type="text" name="name" placeholder="<?php echo Lang::get('api_control.placeholder'); ?>" required
                class="input-dark flex-1">
            <button type="submit" class="btn-primary"><?php echo Lang::get('api_control.generate'); ?></button>
        </form>
        ...
        <div class="flex-1 space-y-4">
            <?php if (empty($keys)): ?>
                <div class="py-12 text-center opacity-30">
                    ...
                    <p class="text-xs font-black uppercase tracking-widest"><?php echo Lang::get('api_control.no_keys'); ?>
                    </p>
                </div>
            <?php else: ?>
    ...
    <section class="glass-card">
        <h2 class="text-xl font-black text-white uppercase tracking-tight mb-8">
            <?php echo Lang::get('api_control.doc_explorer'); ?></h2>
            ...
            <?php foreach ($databases as $db): ?>
                <a href="<?php echo $baseUrl; ?>admin/api/docs?db_id=<?php echo $db['id']; ?>"
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500 mt-1 uppercase font-black tracking-widest">
                            <?php echo Lang::get('api_control.connect_internal'); ?></p>
                    </div>
            ...
            <?php if (empty($databases)): ?>
                <div class="py-12 text-center opacity-30">
                    ...
                    <p class="text-xs font-black uppercase tracking-widest">
                        <?php echo Lang::get('api_control.no_databases'); ?></p>
                </div>
            <?php endif; ?>
        </div>
    </section>
...
<section class="mt-8 glass-card border-emerald-500/20">
    ...
    <div class="prose prose-invert max-w-none text-sm text-slate-400 leading-relaxed font-medium">
        ...
        <p class="mt-4"><?php echo Lang::get('api_control.guide_footer'); ?></p>
    </div>
</section>
...
<script>
    function confirmRevokeKey(id, name) {
        showModal({
            title: '<?php echo Lang::get('api_control.revoke_confirm_title'); ?>',
            message: `<?php echo Lang::get('api_control.revoke_confirm_msg'); ?>`.replace(':name', name),
            type: 'confirm',
            typeLabel: '<?php echo Lang::get('api_control.revoke_confirm_btn'); ?>',
            onConfirm: () => {
