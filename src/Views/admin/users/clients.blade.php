...
@section('content')
    ...
    <header class="text-center mb-10 md:mb-16 relative">
        ...
        <p class="text-p-muted font-medium max-w-2xl mx-auto px-4 text-sm md:text-base">
            Users with Cliente role and their billing information
        </p>
    </header>
    ...
    <div class="mb-8">
        <nav class="flex items-center gap-2 text-xs font-bold text-p-muted">
            <a href="{{ $baseUrl }}admin/users" class="hover:text-primary transition-colors">Users</a>
            <span>&rarr;</span>
            <span class="text-primary">Client Users</span>
        </nav>
    </div>
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
        ...
        <div class="flex items-center gap-4 w-full md:w-auto">
            <form method="GET" class="w-full md:w-auto">
                <input type="text" name="search" value="{{ $_GET['search'] ?? '' }}" placeholder="Search clients..."
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8" id="clientsGrid">
        @forelse($clients as $client)
            <div class="glass-card !p-6 hover:border-primary/30 transition-all group">
                <!-- Client Header -->
                <div class="flex justify-between items-start mb-6">
                    ...
                    <div class="flex gap-2">
                        <a href="{{ $baseUrl }}admin/users/edit?id={{ $client['id'] }}"
                ...
                <div class="grid grid-cols-2 gap-4 mb-6">
                    <div class="bg-white/5 p-3 rounded-xl">
                        <span class="block text-[10px] font-black text-p-muted uppercase tracking-widest mb-1">Projects</span>
                        <span class="text-xl font-black text-p-title">{{ $client['projects_count'] }}</span>
                    </div>
                    <div class="bg-white/5 p-3 rounded-xl">
                        <span class="block text-[10px] font-black text-p-muted uppercase tracking-widest mb-1">Paid</span>
                ...
                <!-- Contact Info -->
                @if($client['phone'])
                    <div class="flex items-center gap-2 text-xs text-p-muted mb-4">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="grid grid-cols-2 gap-2">
                    <a href="{{ $baseUrl }}admin/billing/projects?user_id={{ $client['id'] }}"
                        class="block text-center px-4 py-2 bg-emerald-500/10 hover:bg-emerald-500 hover:text-dark border border-emerald-500/30 rounded-xl text-[10px] font-black uppercase tracking-widest text-emerald-500 transition-all">
                        Projects
                    </a>
                    <a href="{{ $baseUrl }}admin/billing/installments?user_id={{ $client['id'] }}"
                        class="block text-center px-4 py-2 bg-amber-500/10 hover:bg-amber-500 hover:text-dark border border-amber-500/30 rounded-xl text-[10px] font-black uppercase tracking-widest text-amber-500 transition-all">
                        Installments
                    </a>
                </div>
            </div>
        @empty
            <div class="col-span-full glass-card !p-12 text-center">
                ...
                <h3 class="text-xl font-bold text-p-title mb-2">No client users found</h3>
                <p class="text-p-muted mb-6">Create a user with the "Cliente" role to see them here</p>
                <a href="{{ $baseUrl }}admin/users/new"
                    class="btn-primary !px-8 !py-3 font-black uppercase tracking-widest text-xs italic inline-block">
                    Create User
                </a>
            </div>
        @endforelse
