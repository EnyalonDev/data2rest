...
**¿Puedo usar múltiples Client IDs con el mismo Backend?**
**SÍ. Totalmente.** Data2Rest está diseñado para eso.
...
1.  **Grupo de Proyectos (1-50):** Crea una sola credencial OAuth en Google llamada "Grupo 1".
...
2.  **Siguientes 50:** Cuando llenes el cupo de URLs, crea una nueva credencial "Grupo 2".
...
2.  **Data2Rest Admin (`/admin/projects`):**
...
El botón debe redirigir al Backend forzando la URL de retorno:
```tsx
const handleLogin = () => {
...
**3. Callback (`src/pages/auth/GoogleCallback.tsx`)**
El verificador debe enviar `code` Y `redirect_uri`:
```tsx
// ... imports ...
...
**4. Configuración del Servidor Web (`vercel.json` o `.htaccess`)**
