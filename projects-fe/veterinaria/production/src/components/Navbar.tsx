    ...
    <nav className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white shadow-md py-3' : 'bg-transparent py-5'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          >
            <span className={`font-extrabold text-2xl tracking-tight transition-colors ${isScrolled ? 'text-vibrant-main' : 'text-vibrant-main'
              }`}>
              {UI_STRINGS.id_app_name}
            </span>
            <span className={`text-xs font-semibold uppercase tracking-widest ${isScrolled ? 'text-vibrant-dark/70' : 'text-vibrant-dark/80'
              }`}>
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            {isAuthenticated ? (
              <Link
                href="/dashboard"
                className="bg-vibrant-dark text-white px-6 py-2.5 rounded-full font-bold text-sm hover:scale-105 transition-all flex items-center gap-2 shadow-lg shadow-vibrant-dark/20"
              >
      ...
      <div className={`md:hidden absolute w-full bg-white shadow-xl transition-all duration-300 overflow-hidden ${isOpen ? 'max-h-screen pb-10' : 'max-h-0'
        }`}>
